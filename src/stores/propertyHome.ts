...
export const usePropertyStore = defineStore('property', () => {
  ...
  const saveProperty = async (property: Partial<IProperty>) => {
    ...
    if (propertyId > 0) {
      ...
      if (unitId > 0) {
        console.error(
          'PROPERTY UNIT SANS ID AU MOMENT SAVE =',
          normalizedProperty
        );
  ...
  const selectProperty = async (
    ...
    try {
      ...
      if (property.id && Number(property.id) > 0) {
        ...
        const unitId =
          (data as any).unit_id ?? (property as any).unit_id ?? null;
      ...
      selectedProperty.value = {
        ...
        row_type: (property as any).row_type || (unitId ? 'unit' : 'address'),
  ...
  function loadPropertyBaseData(property: any) {
    ...
    selectedProperty.value = {
      ...
      row_type: property.row_type || (unitId ? 'unit' : 'address'),
  ...
  const toggleFavorite = async (
    ...
    if (rowType === 'address') {
      const propertyToCreate: any = {
        ...
        property_type:
          sourceRow?.property_type || sourceRow?.type_code || 'immeuble',
    ...
    throw new Error(
      'Impossible de déterminer le type de ligne pour le favori.'
    );
  };
  ...
  const loadFavoriteAddresses = async (scope?: 'agency' | 'personal') => {
    const rows = await PropertyService.getFavoriteAddresses(scope);
